...
global.WebSocket = jest.fn().mockImplementation((url) => {
  ...
  mockWebSocketInstance = {
    ...
    close: jest.fn(() => {
      ...
      if (mockWebSocketInstance.onclose) {
        mockWebSocketInstance.onclose(new CloseEvent('close', { code: 1000, reason: 'Normal closure' }));
      }
    }),
    ...
    removeEventListener: jest.fn((event, listener) => {
      ...
      if (event === 'open' && mockWebSocketInstance.onopen === listener) mockWebSocketInstance.onopen = null;
      if (event === 'message' && mockWebSocketInstance.onmessage === listener) mockWebSocketInstance.onmessage = null;
      if (event === 'error' && mockWebSocketInstance.onerror === listener) mockWebSocketInstance.onerror = null;
      if (event === 'close' && mockWebSocketInstance.onclose === listener) mockWebSocketInstance.onclose = null;
    }),
  ...
  Object.defineProperty(mockWebSocketInstance, 'CONNECTING', { value: 0, writable: false });
  Object.defineProperty(mockWebSocketInstance, 'OPEN', { value: 1, writable: false });
  Object.defineProperty(mockWebSocketInstance, 'CLOSING', { value: 2, writable: false });
  Object.defineProperty(mockWebSocketInstance, 'CLOSED', { value: 3, writable: false });
...
describe('MultimodalLiveClient Session Resumption', () => {
  ...
  test('WebSocket URL should include sessionId on reconnection after it was received', async () => {
    ...
    await mockWebSocketInstance.simulateServerMessage({ setupComplete: { sessionId: sid } });
    await p;
