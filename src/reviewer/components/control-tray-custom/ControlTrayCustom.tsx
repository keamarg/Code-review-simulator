...
function ControlTray({
  ...
  const audioDataHandler = useCallback(
    (base64: string) => {
      // Use the actual sample rate from AudioRecorder's AudioContext
      // This matches the MediaStream's sample rate (Firefox requirement)
      // Both AudioRecorder and AudioStreamer now use the same AudioContext ("audio-out")
      const sampleRate =
        audioRecorder.audioContext?.sampleRate || audioRecorder.sampleRate || 24000;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [client], // audioRecorder properties are stable and don't need to be in deps
  );
